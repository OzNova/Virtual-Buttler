...
TOOLS: List[Tool] = [
    ...
    Tool("memory.recall", "Recall past conversation notes.",
         {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
         ["what did I ask about budgets?"]),
    Tool("docs.search", "Search allowlisted local documents.",
         {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
         ["find the Q3 marketing budget in my documents"]),
    Tool("docs.summarize", "Extractive summary of the best-matching doc.",
         {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
         ["summarize the Q3 marketing budget pdf"]),
    Tool("vision.capture", "Screenshot the screen; describe with vision LLM if key set.",
         {"type": "object", "properties": {"question": {"type": "string"}}},
         ["why is this script throwing an error?"]),
    Tool("calendar.next", "List upcoming calendar events (opt-in ICS/icalBuddy).",
         {"type": "object", "properties": {}}, ["what is my next meeting?"]),
    Tool("focus.check", "Suggest focus mode when deep-work block is near.",
         {"type": "object", "properties": {}}, ["should I focus now?"]),
    Tool("home.state", "Read a Home Assistant entity state (opt-in).",
         {"type": "object", "properties": {"entity_id": {"type": "string"}}, "required": ["entity_id"]},
         ["is the living room light on?"]),
    Tool("home.call", "Call a Home Assistant service (opt-in).",
         {"type": "object", "properties": {
